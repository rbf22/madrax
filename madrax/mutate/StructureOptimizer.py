...
def optimize(model, coords, info_tensors, epochs=50, verbose=False, learning_rate=0.1, backbone_rotation=False):
    ...
    model : madrax.ForceField A Madrax ForceField object.
        It can be generated using the madrax.ForceField function

    coords : torch.Tensor shape (Batch, nAtoms, 3)
        coordinates of the proteins. It can be generated using the madrax.utils.parsePDB function

    info_tensors : tuple
        a set of precalculated information tensors required by the forcefield. It can be created out
        of the box with the function madrax.dataStructures.create_info_tensors starting from the atom names (that can be
        obtained, along with the coordinates, by the  madrax.utils.parsePDB function)

    epochs : int
        number of optimization epochs default = 50

    learning_rate : float
        learning rate of the optimization default = 0.1 verbose : bool if you wanna see a lot of
        text everywhere in your terminal

    backbone_rotation : bool
       if you want to change the backbone structure of the protein
    ...
    atom_number, atom_description, coordsIndexingAtom, \
        partnersIndexingAtom, angle_indices, alternativeMask = info_tensors
