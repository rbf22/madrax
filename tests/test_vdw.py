...
def test_vdw_energy():
    ...
    # Check that the output tensors have the correct shape (batch, chain, res, altern)
    assert residueEnergyMC.shape == (1, 1, 1, 1)
    assert residueEnergySC.shape == (1, 1, 1, 1)

    # Check the calculated energy values
    assert torch.allclose(residueEnergyMC[0, 0, 0, 0], expected_mc_energy)
    assert torch.allclose(residueEnergySC[0, 0, 0, 0], expected_sc_energy)
